...
<span v-if="data.file_flag === 0" @click="into_file(data.file_id,data.file_name,data.type_flag)" class="files_label">{{ node.label }} </span>
...
<i v-if="!(data.file_flag === 0) && data.file_id != 0" class="el-icon-folder-add" @click="start_create(data)"></i>
  </span>
</el-tree>
...
export default {
  ...
  methods: {
    ...
    rename_file(){
        ...
        {
            console.log('重命名团队文件');
            if (this.file_now.type_flag === 1) //如果是团队文件
            {
                let rename_file_ifo = {
            proj_id:'',
    ...
    copy_file() {
        ...
        if (this.file_now.file_flag === 0) //复制文件
        {
            if (this.file_now.type_flag === 1) //如果是团队文件
            {
                let copy_file_ifo = {
    ...
    create_file(){
        ...
        var is_root;
        if (this.file_now.file_id == 0) is_root = 1; else is_root = 2;
